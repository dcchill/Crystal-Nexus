...
public class PurificationRecipe implements Recipe<RecipeInput> {
	...
	public PurificationRecipe(ItemStack output, NonNullList<Ingredient> recipeItems, List<Integer> integers) {
		this.output = output;
	...
	public static class Type implements RecipeType<PurificationRecipe> {
		...
		public static final RecipeType<PurificationRecipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<PurificationRecipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<PurificationRecipe> CODEC = RecordCodecBuilder
				...
				}, DataResult::success).forGetter(recipe -> recipe.recipeItems), Codec.INT.listOf().fieldOf("integers").forGetter(recipe -> recipe.integers)).apply(builder, PurificationRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, PurificationRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);
		...
		private static PurificationRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new PurificationRecipe(ItemStack.STREAM_CODEC.decode(buf), inputs, numbers);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, PurificationRecipe recipe) {
