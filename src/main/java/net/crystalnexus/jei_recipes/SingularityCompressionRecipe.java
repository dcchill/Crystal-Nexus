...
public class SingularityCompressionRecipe implements Recipe<RecipeInput> {
	...
	public SingularityCompressionRecipe(ItemStack output, NonNullList<Ingredient> recipeItems, List<Integer> integers, List<String> strings) {
		this.output = output;
	...
	public static class Type implements RecipeType<SingularityCompressionRecipe> {
		private Type() {
		}

		public static final RecipeType<SingularityCompressionRecipe> INSTANCE = new Type();
	}

	public static class Serializer implements RecipeSerializer<SingularityCompressionRecipe> {
		public static final Serializer INSTANCE = new Serializer();
		private static final MapCodec<SingularityCompressionRecipe> CODEC = RecordCodecBuilder
				.mapCodec(builder -> builder.group(ItemStack.STRICT_CODEC.fieldOf("output").forGetter(recipe -> recipe.output), Ingredient.CODEC_NONEMPTY.listOf().fieldOf("ingredients").flatXmap(ingredients -> {
					...
					if (aingredient.length == 0) {
						...
						SingularityCompressionRecipe::new));
		public static final StreamCodec<RegistryFriendlyByteBuf, SingularityCompressionRecipe> STREAM_CODEC = StreamCodec.of(Serializer::toNetwork, Serializer::fromNetwork);

		@Override
		public MapCodec<SingularityCompressionRecipe> codec() {
			return CODEC;
		}

		@Override
		public StreamCodec<RegistryFriendlyByteBuf, SingularityCompressionRecipe> streamCodec() {
			return STREAM_CODEC;
		}

		private static SingularityCompressionRecipe fromNetwork(RegistryFriendlyByteBuf buf) {
			...
			return new SingularityCompressionRecipe(ItemStack.STREAM_CODEC.decode(buf), inputs, numbers, strings);
		}

		private static void toNetwork(RegistryFriendlyByteBuf buf, SingularityCompressionRecipe recipe) {
