...
public final class ArcFurnaceRecipeCategory implements IRecipeCategory<ArcFurnaceRecipe> {
	...
	@Override public void setRecipe(IRecipeLayoutBuilder builder, ArcFurnaceRecipe recipe, IFocusGroup focuses) {
		builder.addSlot(RecipeIngredientRole.INPUT, 44, 35).addItemStacks(Arrays.stream(recipe.getIngredients().get(0).getItems())
			.map(stack -> stack.copyWithCount(recipe.ingredientCount(0))).toList());
		if (recipe.getIngredients().size() > 1)
			builder.addSlot(RecipeIngredientRole.INPUT, 26, 35).addItemStacks(Arrays.stream(recipe.getIngredients().get(1).getItems())
				.map(stack -> stack.copyWithCount(recipe.ingredientCount(1))).toList());
