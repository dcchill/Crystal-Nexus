...
public class CircuitPressBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	...
	public Component getDefaultName() {
		return Component.literal("circuit_press");
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new CircuitPressGUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
	...
	private final EnergyStorage energyStorage = new EnergyStorage(MachineAge.from(getBlockState()).minimumCapacity(CrystalnexusConfig.MACHINES.CIRCUIT_PRESS.capacity(), getBlockState().is(CrystalnexusModBlocks.TITANIUM_CARBIDE_CIRCUIT_PRESS.get()) ? 2048 * 8 : 2048), CrystalnexusConfig.MACHINES.CIRCUIT_PRESS.maxReceive(), CrystalnexusConfig.MACHINES.CIRCUIT_PRESS.maxExtract(), 0) {
		@Override
