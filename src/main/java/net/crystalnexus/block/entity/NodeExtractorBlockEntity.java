...
public class NodeExtractorBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	private NonNullList<ItemStack> stacks = NonNullList.withSize(1, ItemStack.EMPTY);

	public NodeExtractorBlockEntity(BlockPos position, BlockState state) {
		super(CrystalnexusModBlockEntities.NODE_EXTRACTOR.get(), position, state);
	}
	...
	public Component getDefaultName() {
		return Component.literal("node_extractor");
	}

	@Override
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new NodeExtractorGUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
