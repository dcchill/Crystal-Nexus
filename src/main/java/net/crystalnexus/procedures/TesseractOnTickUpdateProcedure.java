...
public class TesseractOnTickUpdateProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		double energy = 0;
		double fluid = 0;
		if (CrystalnexusModBlocks.TESSERACT_OUTPUT
				.get() == (world.getBlockState(BlockPos.containing((itemFromBlockInventory(world, BlockPos.containing(x, y, z), 0).copy()).getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("linkX"),
						...
						(itemFromBlockInventory(world, BlockPos.containing(x, y, z), 0).copy()).getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("linkZ")))).getBlock()) {
			if (canReceiveEnergy(world,
					...
					null)) {
				energy = extractEnergySimulate(world, BlockPos.containing(x, y, z), 81920, null);
