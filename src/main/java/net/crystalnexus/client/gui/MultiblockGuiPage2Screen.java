...
public class MultiblockGuiPage2Screen extends AbstractContainerScreen<MultiblockGuiPage2Menu> implements CrystalnexusModScreens.ScreenAccessor {
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
		...
		guiGraphics.blit(ResourceLocation.parse("crystalnexus:textures/screens/reactionsprite.png"), this.leftPos + 6, this.topPos + 7, 0, Mth.clamp((int) MultiblockGuiPage1WhileThisGUIIsOpenTickProcedure.execute(world, x, y, z) * 128, 0, 256), 128,
				128, 128, 384);
	...
	public void init() {
		super.init();
		button_empty = Button.builder(Component.translatable("gui.crystalnexus.multiblock_gui_page_2.button_empty"), e -> {
			int x = MultiblockGuiPage2Screen.this.x;
			int y = MultiblockGuiPage2Screen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new MultiblockGuiPage2ButtonMessage(0, x, y, z));
				MultiblockGuiPage2ButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}).bounds(this.leftPos + 96, this.topPos + 142, 30, 20).build();
		this.addRenderableWidget(button_empty);
		button_empty1 = Button.builder(Component.translatable("gui.crystalnexus.multiblock_gui_page_2.button_empty1"), e -> {
			int x = MultiblockGuiPage2Screen.this.x;
			int y = MultiblockGuiPage2Screen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new MultiblockGuiPage2ButtonMessage(1, x, y, z));
				MultiblockGuiPage2ButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
