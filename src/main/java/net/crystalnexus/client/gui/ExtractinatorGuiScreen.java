...
import net.crystalnexus.procedures.ProgressDisplay6Procedure;
...
public class ExtractinatorGuiScreen extends AbstractContainerScreen<ExtractinatorGuiMenu> implements CrystalnexusModScreens.ScreenAccessor {
	private final Level world;
	private final int x, y, z;
	private final Player entity;
	private boolean menuStateUpdateActive = false;

	public ExtractinatorGuiScreen(ExtractinatorGuiMenu container, Inventory inventory, Component text) {
	...
	private static final ResourceLocation texture = ResourceLocation.parse("crystalnexus:textures/screens/extractinator_gui.png");
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > leftPos + -22 && mouseX < leftPos + 2 && mouseY > topPos + 9 && mouseY < topPos + 33) {
			String hoverText = ExtractinatorOnTickUpdateProcedure.execute(world, x, y, z);
			if (hoverText != null) {
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
		...
		guiGraphics.blit(ResourceLocation.parse("crystalnexus:textures/screens/extractinator_bar.png"), this.leftPos + 55, this.topPos + 6, 0, Mth.clamp((int) ProgressDisplay6Procedure.execute(world, x, y, z) * 64, 0, 320), 64, 64, 64, 384);
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.crystalnexus.extractinator_gui.label_proc_get_block_name_for_gui"), 74, -10, -12829636, false);
	}
